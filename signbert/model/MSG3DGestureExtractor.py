...
class HandsGraph:
    """
    Класс в котором хранится структура рук с 21 кипоинтами

    Этот класс создаёт графовое представление руки, с нодами, которые представляют кипоинты
    и рёбра представляют собой соединения между кипонтами

    Attributes:
        num_nodes(int): Число нод(кипоинтов) в графе
        edges(list): Список рёбер, показывающий соединения между кипоинтами
        self_loops (list): Список самопроизвольных циклов для каждого узла в графе
        A_binary(ndarray): Бинарная матрица смежности графа без циклов
        A_binary_with_I (ndarray): Бинарная матрица смежности графа с циклами.
    """
    def __init__(self, *args, **kwargs):
        self.num_nodes = 21
        # Инициализируем рёбра графа, основываясь на связи кипоинтов рук
        inward = [
            (0, 1), (0, 5), (0, 17),  # Запястье
            (5, 9), (9, 13), (13, 17),  # Ладонь
            (1, 2), (2, 3), (3, 4),  # Большой палец
...
class GestureExtractor(nn.Module):
    ...
    def __init__(
        ...
        self.model = MSG3D(
            ...
            num_gcn_scales,
            num_g3d_scales,
            HandsGraph(),  # Feasibility study graph
            hid_dim,
            msg_3d_dropout,
            in_channels,
        )
